...
function AsciiGlobe({
  ...
  // Animation loop
  useEffect(() => {
    const FPS = 12; // Lower FPS for globe (more complex)
    const frameInterval = 1000 / FPS;
    const STEP = 0.006; // Slightly larger step = fewer iterations (was 0.004)
    ...
    const render = (timestamp: number) => {
      ...
      // Throttle frame rate
      if (timestamp - lastRenderRef.current < frameInterval) {
        return;
      }
      lastRenderRef.current = timestamp;

      // After first render, only animate when visible
      if (hasRenderedOnceRef.current && !isVisibleRef.current) {
        return;
      }
